...
subroutine EOS_Omni_EOS_short(eoskey,keytemp,rf_precision,npoints,&
  ...
  if(keytemp.eq.1) then
     call nuc_eos_m_kt1_short(npoints,rho,temp,ye,eps,press,&
          entropy,cs2,dedt,dpderho,dpdrhoe,munu,keyerr,anyerr,EOS_cs_is_nonrelativistic)
...
subroutine EOS_Omni_EOS_full(eoskey,keytemp,rf_precision,npoints,&
  ...
  write(warnstring,"(A8,i5)") "NUNO eoskey: ", eoskey
  if(eoskey.ne.4) then
     !$OMP CRITICAL
     write(warnstring,"(A8,i5)") "NUNO warn eoskey: ", eoskey
     call CCTK_WARN(1,warnstring)
  ...
  if(keytemp.eq.1) then
     call nuc_eos_m_kt1_full(npoints,rho,temp,ye,eps,press,&
          ...
          mue,mun,mup,muhat,keyerr,anyerr,EOS_cs_is_nonrelativistic)
  else if(keytemp.eq.0) then
...
subroutine EOS_Omni_EOS_dpderho_dpdrhoe(eoskey,keytemp,rf_precision,npoints,&
  ...
  real*8           :: hybrid_local_k, hybrid_local_eps
  real*8           :: hybrid_dp_poly,hybrid_dp_th
  ...
  case (3)
     ! hybrid EOS                                                                                          
        do i=1,npoints
           ...
           xdpdrhoe = hybrid_dp_poly + max(0.0d0,hybrid_dp_th)
     ...
     case (4)
        ...
        if(keytemp.eq.1) then
           call nuc_eos_m_kt1_press_eps_cs2(npoints,rho,temp,ye,&
                eps,press,cs2,keyerr,anyerr,EOS_cs_is_nonrelativistic)
        else if(keytemp.eq.0) then
           call nuc_eos_m_kt0_press_cs2(npoints,rho,temp,ye,eps,press,&
                cs2,rf_precision,keyerr,anyerr,EOS_cs_is_nonrelativistic)
        else
