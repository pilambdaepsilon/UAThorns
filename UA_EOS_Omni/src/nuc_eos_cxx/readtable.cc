...
#define READ_BCAST_EOS_HDF5(NAME,VAR,TYPE,MEM,NELEMS)                         \
  do {                                                                        \
    hid_t dataset;                                                            \
    HDF5_DO_IO(dataset = H5Dopen(file, NAME));                                \
...
#define READ_BCAST_EOSTABLE_HDF5(NAME,OFF,DIMS)                          \
  ...
  printf("Done allocating table mem \n");
  ...
  if(full_eos_info){
  ...
  else{// in case some EOS info is missing, substitute it with data for other variables -- this is not recommended and can lead to unphysical 
       // results. Use only if you know the substitued information is NEVER used or can't affect the state of other vaiables
     READ_BCAST_EOSTABLE_HDF5("logpress",  0, table_dims);
     READ_BCAST_EOSTABLE_HDF5("logenergy", 1, table_dims);
     READ_BCAST_EOSTABLE_HDF5("entropy",   2, table_dims);
     READ_BCAST_EOSTABLE_HDF5("mu_e",      3, table_dims);
     READ_BCAST_EOSTABLE_HDF5("cs2",       4, table_dims);
     READ_BCAST_EOSTABLE_HDF5("dpdrhoe",   5, table_dims);
     READ_BCAST_EOSTABLE_HDF5("dpdrhoe",   6, table_dims);
     READ_BCAST_EOSTABLE_HDF5("dpdrhoe",   7, table_dims);
     // chemical potentials
     READ_BCAST_EOSTABLE_HDF5("mu_n",     8, table_dims);
     READ_BCAST_EOSTABLE_HDF5("mu_e",      9, table_dims);
     READ_BCAST_EOSTABLE_HDF5("mu_p",     10, table_dims);
     READ_BCAST_EOSTABLE_HDF5("mu_n",     11, table_dims);
     // compositions
     ...
     READ_BCAST_EOSTABLE_HDF5("Zbar",     17, table_dims);
     // Gamma
     READ_BCAST_EOSTABLE_HDF5("Zbar",    18, table_dims);
  ...
  if (!(presstable = (double*)malloc(nrho * ntemp * nye
				    * sizeof(double)))) {
    CCTK_VError(__LINE__, __FILE__, CCTK_THORNSTRING,
                "Cannot allocate memory for press table\n");
  }
